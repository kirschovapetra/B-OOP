...
public class PNCanvas extends Canvas implements MouseListener {
    ...
    private ButtonAction clickedButton; //enum = ktory button bol kliknuty
    ...
    public ButtonAction getClickedButton() {
        return clickedButton;
    }
    public void setClickedButton(ButtonAction clickedButton) {
    ...
    //sleduje sa kliknutie mysou
    @Override
    public void mouseClicked(MouseEvent e) {
        clicks.add(e);

        if (clickedButton == ButtonAction.ADD_PLACE){
            addPlaceMode();
        }
        if (clickedButton == ButtonAction.ADD_TRANSITION){
            addTransitionMode();
        }
        if (clickedButton == ButtonAction.ADD_REGULAR_EDGE){
            addRegularEdgeMode();
        }
        if (clickedButton == ButtonAction.ADD_RESET_EDGE){
            addResetEdgeMode();
        }
        if (clickedButton == ButtonAction.REMOVE){
            removeMode();
        }
        if (clickedButton == ButtonAction.SIMULATION){
            simulationMode();
        }
    ...
    public void addPlaceMode() {
        ...
        if (clicksCount > oldClicksCount) { //ak bolo nove kliknutie
            ...
            int x = lastClick.getX() - SIZE / 2;
            int y = lastClick.getY() - SIZE / 2;
